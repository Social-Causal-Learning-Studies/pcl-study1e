...
function make_slides(f) {
  ...
  slides.causalMatrixH = slide({
    ...
    button : function() {
      ...
      if($('input[name=blue]:checked').length == 0|$('input[name=pink]:checked').length == 0|$('input[name=both]:checked').length == 0|$('input[name=bp]:checked').length == 0|$('input[name=pb]:checked').length == 0|$('input[name=orange]:checked').length == 0|$('input[name=bothOrange]:checked').length == 0|$('input[name=ob]:checked').length == 0|$('input[name=all]:checked').length == 0|$('input[name=other]:checked').length == 0){
        $(".err").show();
      } else {
...
function sendDataToServer(data) {
	$.ajax({
    url: 'https://script.google.com/macros/s/AKfycbwZ0YoDIlwKac2AKXV0MpfBVQx5cC9XKeCLedJ90qB7CL2YdXSafEZHL_EBl96yVBZ4gA/exec',
		type: 'post',
    data: data,
    complete: function(){
      window.location.replace("https://app.prolific.co/submissions/complete?cc=323C3655") // redirect to prolific
    } 
	});
}
